...
def RealtimePloter():
  ...
  plot1.line1[0].set_data(CurrentXAxis,pylab.array(values[-NumberSamples:]))
  plot1.ax.axis([CurrentXAxis.min(),CurrentXAxis.max(),-1.5,1.5])
  plot1.canvas.draw()
...
def plot1():
    ...
    fig = pylab.figure(1)
    plot1.ax = fig.add_subplot(111)
    plot1.ax.grid(True)
    plot1.ax.set_title("Realtime Waveform Plot")
    plot1.ax.set_xlabel("Time")
    plot1.ax.set_ylabel("Amplitude")
    plot1.ax.axis([0,100,-1.5,1.5])
    plot1.line1=plot1.ax.plot(xAchse,yAchse,'-')

    plot1.canvas = FigureCanvasTkAgg(fig, master=root)
    plot1.canvas.draw()
    plot1.canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)

    toolbar = NavigationToolbar2Tk( plot1.canvas, root )
    toolbar.update()
    plot1.canvas._tkcanvas.pack(side=tk.TOP, fill=tk.BOTH, expand=1)
...
wScale = tk.Scale(master=root,label="View Width:", from_=3, to=1000,sliderlength=30, orient=tk.HORIZONTAL)
wScale2 = tk.Scale(master=root,label="Generation Speed:", from_=1, to=200,sliderlength=30, orient=tk.HORIZONTAL)
...
#Draw button
graph_button = tk.Button(root, text="Execute",bg='brown',fg='white', command=plot1)
